function [pk] = Kernel_Compute_v1asDV(out)
...
for n = 1:nneuron
    ...
    dv = dv(:,end);
    med = median(dv);
    disp(['V1 neuron ' num2str(n) ' ...median confidence: ' num2str(med)])
    ...
    for k = 1:nbin
        ...
        tkernel_h_each(:,k) = getKernel(hdxmat(dv > med, begin:begin+frameperbin-1), ch(dv > med));
        tkernel_l_each(:,k) = getKernel(hdxmat(dv < med, begin:begin+frameperbin-1), ch(dv < med));
