...
%% plot CP as a function of time
close all;
figure;
% subplot(1,2,1)
me = mean(CP_half,1);
sem = std(CP_half, [], 1)/sqrt(neurons);
fill([1:time fliplr(1:time)],[me - sem fliplr(me + sem)], 0.45*ones(1,3));
hold on;
plot(1:time, me, '-k','linewidth',2)
xlabel('time')
ylabel('CP')
...
% subplot(1,2,2)
% start = 5;
% plot(mean(mean(CP_half(:, start:n0S),2)),'-');
% plot(mean(mean(CP_half(:,n0S+1:time),1)),'r-');
% xlabel('')
% ylabel('CP')
% set(gca, 'box','off'); set(gca, 'TickDir', 'out')
